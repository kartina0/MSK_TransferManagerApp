...
namespace ServerModule
{
    ...
    public class OrderInfo_IF_DB : IOrderInfo
    {
        ...
        private string GetOrderItemSelectSql(string tableName, DateTime orderDateTime, int postNo, Const.ORDER_PROCESS process)
        {
            ...
            if (process != Const.ORDER_PROCESS.NONE)
            {
                result.Append($"AND ");
        ...
        private string GetOrderItemMaxSql(string tableName, Const.ORDER_PROCESS process)
        {
        ...
        private void DataToOrderItem(Dictionary<string, object> data, out OrderData orderData)
        {
            try
            {
                ...
                orderData = new OrderData
                {
                    ...
                    process = (Const.ORDER_PROCESS)Convert.ToInt32(data[ColumnNamePickHead.PROCESS]),
        ...
        public UInt32 SetExecuteItem(string tableName, ExecuteData item)
        {
            ...
            try
            {
                ...
                updateSql.Append($"{ColumnNameCompHead.UPDATE_LOGIN_ID} = '{item.updateLoginId}' ");
